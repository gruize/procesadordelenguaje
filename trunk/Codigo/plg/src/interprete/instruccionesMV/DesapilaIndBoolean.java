...
public class DesapilaIndBoolean extends InstruccionMaquinaP{
	...
	public int exec(Stack<StackObject> p, Memoria m, Integer counter) {
		...
		if (v instanceof MyBoolean){
			if (p.isEmpty()){
				p.push(new MyExecutionError(MyExecutionError.STACK_ERROR,"Stack is empty(2)"));
				return -1;
			}
			else{
				StackObject d = p.pop();
	...
	int pos = 0;
	bytes[pos++] = InstruccionMaquinaP.DESAPILA_IND_BOOLEAN;
	return bytes;
	}

	public static DesapilaIndBoolean fromBytes(byte[] bytes, int pos){
		if (bytes[pos++]!= InstruccionMaquinaP.DESAPILA_IND_BOOLEAN){
			return null;
		}
		return new DesapilaIndBoolean();
	}
	public String toString(){
		return "Code="+InstruccionMaquinaP.DESAPILA_IND_BOOLEAN+". desapila_ind";
	}
